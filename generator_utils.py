...
def decode(encoded_sparql):
    ...
    return ' '.join(convert_abbr_to_uri(sparql))


def convert_abbr_to_uri(sparql):
    sparql = sparql.split()
    for i, token in enumerate(sparql):
        for abbr in NAMESPACE_PREFIXES:
            pref = '<' + abbr[0]
            if token.startswith(pref):
                sparql[i] = '<' + abbr[1] + sparql[i][len(pref):]
    return sparql
